...
an agent and hoping it writes the right files.
    ...
    python install.py --apply --init-project .   # ...and scaffold .workflow/ here
...
def _merge_managed(existing: str, incoming: str, start: str, end: str) -> tuple[str, str]:
    """Splice the incoming managed block into `existing`. Returns (result, how)."""
    block = _managed_block(incoming, start, end)
    if block is None:
        # Nothing marked in the source: treat the whole file as the payload rather than
...
def _install_text(src: Path, dest: Path, key: str, plan: Plan, apply: bool,
                  backup_root: Path, project_root: Path | None) -> None:
...
def _install_settings(src: Path, dest: Path, plan: Plan, apply: bool,
    ...
    _backup(dest, backup_root, plan, apply)
    plan.add("merge", dest, f"add {', '.join(added)}")
    if apply:
...
def _targets() -> list[tuple[Path, Path, str]]:
    ...
    mapping = [
        (DIST_CONFIG / "claude" / "CLAUDE.md", HOME / ".claude" / "CLAUDE.md", "claude/CLAUDE.md"),
        (DIST_CONFIG / "opencode" / "AGENTS.md", HOME / ".config" / "opencode" / "AGENTS.md", "opencode/AGENTS.md"),
    ]
    for sub, dest_dir in (("skills", HOME / ".claude" / "skills"),
                          ("hooks", HOME / ".claude" / "hooks")):
        source_dir = DIST_CONFIG / "claude" / sub
        if source_dir.is_dir():
            for path in sorted(source_dir.iterdir()):
                if path.is_file():
                    mapping.append((path, dest_dir / path.name, f"claude/{sub}/{path.name}"))
...
def main() -> int:
    ...
    parser.add_argument("--init-project", metavar="DIR", help="also scaffold .workflow/ in DIR")
    args = parser.parse_args()
    apply = args.apply

    if not MANIFEST.exists():
        print("[INSTALL] dist/manifest.json missing — run tools/extract_config.py first")
        return 1
    ...
    plan = Plan()
    print(f"[INSTALL] agent-workflow v{manifest.get('version')} "
    ...
    if settings_src.exists():
        _install_settings(settings_src, HOME / ".claude" / "settings.json", plan, apply, backup_root)

    # AGENT_PATH is the bootstrap pointer for `init` in projects that have no .workflow yet.
    agent_path = REPO_ROOT / "main.py"
    if os.environ.get("AGENT_PATH") != str(agent_path):
        plan.add("env", "AGENT_PATH", f"set to {agent_path} (shell command printed below)")
    ...
    if os.environ.get("AGENT_PATH") != str(agent_path):
        ...
        if os.name == "nt":
            print(f'    [Environment]::SetEnvironmentVariable("AGENT_PATH","{agent_path}","User")')
        else:
