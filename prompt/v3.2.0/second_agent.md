# Second Agent — v3.2.0 Setup Prompt (OpenCode, Read-Only)

> Paste prompt ini ke OpenCode.
> Agent generate file `~/.config/opencode/AGENTS.md` persis seperti spec di bawah.
> Prompt ini khusus untuk setup second_agent (OpenCode) pada arsitektur v3.2.0.
...
Buat file berikut di `~/.config/opencode/`:
...
1. Tulis konten `AGENTS.md` persis seperti block di bawah.
2. Jika file sudah ada → backup ke `AGENTS.md.bak` dulu, baru overwrite.
3. Tidak boleh tambah file lain. Tidak boleh ubah konten.
...
- `explore`  → graphify map + targeted file reads
...
5. Structured block labels `[EVIDENCE]`, `[PLAN]` dst: TETAP
