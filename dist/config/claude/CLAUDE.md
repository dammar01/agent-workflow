...
- TASK BUDGET: <task> = INSTRUKSI ringkas (aman ≤3000 char di semua provider; cap SEBENARNYA diturunkan runtime dari transport provider — argv (agy) dapat sisa ruang command line yang terukur, file (opencode, prompt dilampirkan `-f`) dan stdin (codex) memakai `DEFAULT_MAX_TASK_CHARS`. Nilai efektif ada di `meta.task_cap` + `meta.task_cap_source`; truncate senyap di atasnya). JANGAN tempel evidence/dump/isi file ke task — itu tugas second_agent gather, bukan isi prompt. Task kepanjangan → RINGKAS instruksinya, JANGAN pre-split buta jadi 2 call pakai angka argv (8191). Response bawa `meta.task_truncated` bila kena cap — itu sinyal ringkas, bukan izin split.
...
- /.verify-browser = LOCAL, terpisah dari /.verify. Baca section `e2e` di config.json → wawancara HANYA bila belum dikonfigurasi → draft spec (second_agent) → langsung run browser Playwright → review second_agent → [VERIFICATION] canonical. DILARANG tanya "jalankan atau tidak"; yang masih menghentikan cuma draft blocked/invalid, missing_env, dan izin read_only_requests yang belum diberikan. Default headed (`headless: false`). Nama `.test` lolos tanpa `allow_remote`; domain sungguhan tetap butuh opt-in; write dinilai dari alamat hasil resolve lalu dipatok ke browser. Run mengusulkan tag `data-e2e` untuk elemen ber-`ref` `path:line` — runtime nol tulis, user konfirmasi satu batch, lalu /.promote. Setting = config project + override per-run di request session; credential HANYA nama key + nama profil `.workflow/e2e/secrets.json`, nilai DILARANG lewat chat. Detail: skill verify-browser.
...
"kenapa X lambat" itu analyze walau soal performa, "di mana X" itu explore walau soal bug.
  ...
  verify-browser ← test di browser | cek di browser | uji ui | playwright | e2e browser
             ...
             INTI: pembuktian lewat browser sungguhan. Project sudah punya section `e2e`
             di config.json → jalan tanpa wawancara, tanpa gate konfirmasi.
